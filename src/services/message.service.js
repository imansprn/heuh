// -----------------------------------------------------------------------------
//  message.service.js  --  HEUH v4.0
//
//  GitSync-style layout via Columns fake header:
//    col LEFT  (FILL_AVAILABLE_SPACE) : avatar icon + repo name
//    col RIGHT (FILL_MINIMUM_SPACE)   : filled colored badge button
//
//  Card structure per event:
//    section[0]  fake-header  Columns widget
//    section[1]  body         PR title, branch, CI+comments, body snippet
//    section[2]  diff image   (injected by injectDiffImage)
//    section[-1] buttons
// -----------------------------------------------------------------------------
...
const SENTRY_META = {
    critical: { label: 'CRITICAL', hex: '#6c1111', icon: '🔥' },
    error:    { label: 'ERROR',    hex: '#922b21', icon: '✖'  },
    warning:  { label: 'WARNING',  hex: '#784212', icon: '⚠'  },
    info:     { label: 'INFO',     hex: '#1e3a5f', icon: 'ℹ'  },
    unknown:  { label: 'UNKNOWN',  hex: '#555555', icon: '❓' },
};
...
/** #rrggbb -> Google Chat RGBA float object */
const hexToRgba = (hex) => {
...
// -- Fake header via Columns --------------------------------------------------
//
//  LEFT  col: repo icon (iconUrl circle) + repo name as decoratedText
//  RIGHT col: filled disabled badge button (the only way to get a colored pill)
//
//  FILL_MINIMUM_SPACE caps the right col at ~30% so the badge stays compact
//  and the repo name gets the remaining space.
const fakeHeader = (repoFullName, avatarUrl, badgeMeta, badgeUrl = 'https://github.com') => ({
    columns: {
        columnItems: [
            {
                ...
                widgets: [{
                    buttonList: {
                        buttons: [{
                            ...
                            // No disabled:true — Google Chat greys out color when disabled.
                            // onClick is required when color is set; we use a no-op anchor.
                            onClick: { openLink: { url: badgeMeta.url ?? badgeUrl } },
                        }],
                    },
                }],
            },
...
const btn = (items) => ({
    buttonList: {
        buttons: items.map(({ text, url, filled }) => ({
            text,
            onClick: { openLink: { url } },
            ...(filled ? { color: { red: 0.039, green: 0.518, blue: 0.514, alpha: 1 } } : {}),
        })),
    },
});
...
const buildPROpenedCard = ({ pull_request: pr, repository, sender }) => {
    ...
    return makeCard(`pr-opened-${pr.number}`, [
        // [0] fake header
        { widgets: [fakeHeader(repository.full_name, sender.avatar_url, meta, pr.html_url)] },
        // [1] body
        { widgets: [
        ...
        // [-1] buttons
        { widgets: [btn([
            { text: 'View PR',        url: pr.html_url, filled: true },
...
const buildPRMergedCard = ({ pull_request: pr, repository, sender }) => {
    ...
    return makeCard(`pr-merged-${pr.number}`, [
        ...
        { widgets: [btn([{ text: 'View PR', url: pr.html_url, filled: true }])] },
    ]);
};
...
const buildReviewRequestedCard = ({ pull_request: pr, repository, sender, requested_reviewer }) => {
    ...
    return makeCard(`pr-review-req-${pr.number}`, [
        ...
        { widgets: [btn([
            { text: 'Review Now', url: `${pr.html_url}/files`, filled: true },
...
const buildReviewSubmittedCard = ({ pull_request: pr, repository, sender, review }) => {
    ...
    return makeCard(`pr-review-${pr.number}`, [
        ...
        { widgets: [btn([
            { text: 'View Review', url: review?.html_url ?? pr.html_url, filled: true },
            { text: 'View PR',     url: pr.html_url },
        ])] },
    ]);
};
...
const buildSynchronizeCard = ({ pull_request: pr, repository, sender, after }) => {
    ...
    const sha  = (after ?? pr.head.sha ?? '').slice(0, 7);
    return makeCard(`pr-sync-${pr.number}`, [
        { widgets: [fakeHeader(repository.full_name, sender.avatar_url, meta, pr.html_url)] },
        { widgets: [
            prTitleWidget(pr.title),
            branchWidget(pr.head.ref, pr.base.ref),
            { decoratedText: { topLabel: 'HEAD SHA', text: sha } },
        ] },
        { widgets: [btn([
            { text: 'View Changes', url: `${pr.html_url}/files`, filled: true },
            { text: 'View PR',      url: pr.html_url },
        ])] },
    ]);
};
...
const formatGitHubCard = (payload) => {
    ...
    return makeCard('unhandled', [{
        widgets: [fakeHeader(
            ...
            GITHUB_FAVICON,
            { label: action.toUpperCase(), hex: '#57606a' }
        )],
    }, {
...
const formatSentryCard = (payload) => {
    ...
    return makeCard(`sentry-${eventId}`, [
        // [0] fake header: project name left, severity badge right
        { widgets: [fakeHeader(project, SENTRY_FAVICON, meta)] },
        // [1] body
        { widgets: [
        ...
        // [-1] buttons
        { widgets: [btn([{ text: 'View in Sentry', url: issueUrl, filled: true }])] },
    ]);
};

// =============================================================================
//  INJECTION HELPERS  (called by webhook.controller after async fetches)
// =============================================================================

/**
 * Inject CI status + comment count widget into body section (sections[1]).
 * Inserted after the branch row (index 1, 0-based) in the body widgets.
 */
...
/**
 * Insert diff image section before the last section (buttons).
 */
const injectDiffImage = (cardMessage, diffImageUrl) => {
    if (!diffImageUrl) return cardMessage;
    try {
        const sections = cardMessage.cardsV2[0].card.sections;
        sections.splice(sections.length - 1, 0, {
            widgets: [{ image: { imageUrl: diffImageUrl, altText: 'Code diff preview' } }],
        });
    } catch (err) {
        console.error('injectDiffImage error:', err.message);
    }
    return cardMessage;
};
